...
#include <mav_msgs/msgs/actuators.hpp>
...
void GazeboBagPlugin::AttitudeThrustCallback(
    const mav_msgs::msg::AttitudeThrustConstPtr& control_msg) {
  common::Time now = world_->SimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
...
void GazeboBagPlugin::ActuatorsCallback(
    const mav_msgs::msg::ActuatorsConstPtr& control_msg) {
  common::Time now = world_->SimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
...
void GazeboBagPlugin::RateThrustCallback(
    const mav_msgs::msg::RateThrustConstPtr& control_msg) {
  common::Time now = world_->SimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
...
void GazeboBagPlugin::LogMotorVelocities(const common::Time now) {
  ...
  mav_msgs::msg::Actuators rot_velocities_msg;
